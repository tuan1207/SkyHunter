    ...
    execute: function () {
      ...
      _export("JoyStick", JoyStick = (_dec = ccclass('JoyStick'), _dec2 = property(Node), _dec3 = property(Node), _dec4 = property(Node), _dec(_class = (_class2 = (_class3 = class JoyStick extends Component {
        constructor() {
          ...
          this.moveSpeed = 200;
          this.lastDirection = new Vec3(0, 0, 0);
        ...
        onStickMove(event, dt) {
          if (this.isStickMoving) {
            ...
            var distance = Math.abs(math.Vec3.distance(this.VecY, this.Pos));
            var speed = 0.05;
            var posX = this.plane.position.x + direction.x * distance * speed;
            var posY = this.plane.position.y + direction.y * distance * speed;
        ...
        update(dt) {
          if (!this.isStickMoving) {
            var distance = Math.abs(math.Vec3.distance(this.VecY, this.Pos));
            var speed = 0.05;
            var posX = this.plane.position.x + this.lastDirection.x * distance * speed;
            var posY = this.plane.position.y + this.lastDirection.y * distance * speed;
...
//# sourceMappingURL=1a06e07774b13355f5bad48b0972df3edec3b002.js.map