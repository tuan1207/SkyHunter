System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Component, Node, Prefab, Vec3, instantiate, Label, ProgressBar, sys, NodePool, RigidBody2D, Vec2, tween, ButtonPlayScene, _dec, _dec2, _dec3, _dec4, _dec5, _dec6, _dec7, _dec8, _dec9, _class, _class2, _descriptor, _descriptor2, _descriptor3, _descriptor4, _descriptor5, _descriptor6, _descriptor7, _descriptor8, _class3, _crd, ccclass, property, GameCtrl;
    ...
    execute: function () {
      ...
      _export("GameCtrl", GameCtrl = (_dec = ccclass('GameCtrl'), _dec2 = property(Prefab), _dec3 = property(Prefab), _dec4 = property(Prefab), _dec5 = property(Node), _dec6 = property(Label), _dec7 = property(Label), _dec8 = property(Label), _dec9 = property(ProgressBar), _dec(_class = (_class2 = (_class3 = class GameCtrl extends Component {
        constructor(...args) {
          ...
          this.enemyVelos = [new Vec2(0, -7.5), new Vec2(7.5, -5), new Vec2(7.5, 0), new Vec2(-7.5, -5), new Vec2(-7.5, 0)];
        }

        onLoad() {
          GameCtrl.instance = this;
          this.schedule(this.spawn, 6, 9, 0); // this.spawn();
        ...
        spawn() {
          ...
          let bulletPrefab0 = this.enemyWave1[0];
          ...
          for (let j = 0; j < 5; j++) {
            ...
            switch (this.countElement) {
              case 0:
                prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[0];
                prefabInstance.setWorldPosition(this.listPos[0].x + 120 * (j - 2), this.listPos[0].y, this.listPos[0].z);
                break;

              case 1:
                prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[1];
                prefabInstance.setWorldPosition(this.listPos[1].x - 100 * (0 + j), this.listPos[1].y + 60 * (j + 1), this.listPos[0].z);
                break;

              case 2:
                // console.log(this.node.position);
                // if(prefabInstance.position.y > 0){
                // prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[0];
                // }else{
                //     prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[2];
                // }
                let startPoint = new Vec2(0, 320); // Điểm chuyển động theo chiều ngang (200, 0)

                let horizontalPoint = new Vec2(200, 0); // Điểm chuyển động theo chiều dọc (200, -100)

                let verticalPoint = new Vec2(0, 100); // Tạo tween để di chuyển từ điểm xuất phát đến điểm chuyển động theo chiều ngang

                let tween1 = tween().to(1, {
                  position: horizontalPoint
                }); // Tạo tween để di chuyển từ điểm chuyển động theo chiều ngang đến điểm chuyển động theo chiều dọc

                let tween2 = tween().to(1, {
                  position: verticalPoint
                });
                tween(prefabInstance).then(tween2).then(tween1).start();
              ...
              case 4:
                // if(prefabInstance.position.y < 0){
                prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[0]; // }else{
                // prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[4];
                // }
              ...
              case 5:
                // if(prefabInstance.position.y > 60){
                prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[2]; // }else{
                // prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[1];
                // }
              ...
              case 6:
                // if(prefabInstance.position.y > 200){
                prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[0]; // }else{
                // prefabInstance.getComponent(RigidBody2D).linearVelocity = new Vec2(0, 0);
                // }
              ...
              case 7:
                prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[3];
              ...
              case 9:
                // if(prefabInstance.position.y > 60){
                prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[4]; // }else{
                //     prefabInstance.getComponent(RigidBody2D).linearVelocity = this.enemyVelos[3];
                // }
...
//# sourceMappingURL=26a2df1ae0d8f5d1222732bfdc891fbcbdda3e5a.js.map